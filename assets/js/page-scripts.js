...
$('#ctrl-drainage_connection_details').on('change', function(){ 
	//do something like 
	//$(this).hide(); 
	//$(#anotherfieldid).show();
	//$(#anotherfieldname).val(ctrlVal)
});
$('#ctrl-type_of_septic').on('change', function(){ 
	//do something like 
	//$(this).hide(); 
	//$(#anotherfieldid).show(); 
	var cv = $(this).val();
	if(cv=="SELF"){
	    ...
	    $("#ctrl-demand_applicable").val("0");
	    $('#ctrl-no_of_flats').val("0");
	}else{ 
	    $("#ctrl-no_of_flats").parents(".form-group").show();
	    $("#ctrl-demand_applicable").parents(".form-group").show(); 
	}
	//$(#anotherfieldname).val(ctrlVal)
});
$('#ctrl-no_of_flats').on('change', function(){ 
	//do something like 
	//$(this).hide(); 
	//$(#anotherfieldid).show();
	var cv = $(this).val();
	$("#ctrl-demand_applicable").val(parseFloat(cv)*4000);
	//$(#anotherfieldname).val(ctrlVal)
});
$('#ctrl-demand').on('change', function(){ 
    ...
    if(cv == "NO"){
        ...
        $('#ctrl-amount').val('');
        $('#ctrl-amount').removeAttr("readonly");
    }
});
});
/**
