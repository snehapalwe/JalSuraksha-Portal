...
function cert_function($datarr,$pagename){
	$data=[];
	$data[]= ["date"=>date("d M Y")]; 
	foreach($datarr as $key=>$val){ 
  	$dateString = $val; // Replace with your date string

		if (strtotime($dateString) !== false) { 
			  $data["humandate_".$key]=date("d M Y",strtotime($dateString)); 
			  $data["official_date_".$key]=date("d/m/Y",strtotime($dateString)); 
		}  

		 
		if($key=="gender"){ 
			if(round($dateString)==1 || $dateString=="Male" || $dateString=="male"){
				$data['gender_english']="Male";
				$data['gender_marathi']="पुरुष";
			}elseif(round($dateString)==2 || $dateString=="Female" || $dateString=="female"){
				$data['gender_english']="Female";
				$data['gender_marathi']="स्त्री";
			}else{
				$data['gender_english']=$dateString;
				$data['gender_marathi']=$dateString;
			}
		}
	    ...
	    $db=$jei->GetModel();
                                ...
                                $db->where("db_name",$pagename);
                                $db->where("rec_id",$datarr['id']);
                                $if=$db->getOne("inspection_form"); 
                                $data['inspection_date']=human_date( $if['inspection_date'] );
