...
// $record['timestamp'] = human_datetime($record['timestamp']);
	...
	function revert($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id","upload_architech_affidavit","upload_construction_permission_and_cerified_map_copy","upload_architect_or_project_consultant_report","upload_property_location_map","upload_noc_copy_given_for_construction_permission","upload_map_prepared_by_licensed_plumber","status");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'upload_architech_affidavit' => 'required',
		...
		return $this->render_view("drainage_noc_jal_oc/revert.php", $data);
	}
