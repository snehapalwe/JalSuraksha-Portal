<?php 
/**
 * Drainage_noc_jal_cc Page Controller
 * @category  Controller
 */
class Drainage_noc_jal_ccController extends SecureController{
	function __construct(){
		parent::__construct();
		$this->tablename = "drainage_noc_jal_cc";
	}
	/**
     * List page records
...
if(USER_ROLE==5){
	...
	# End of before list statement
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				drainage_noc_jal_cc.id LIKE ? OR 
				drainage_noc_jal_cc.status LIKE ? OR 
				drainage_noc_jal_cc.application_no LIKE ? OR 
				drainage_noc_jal_cc.applicant_name LIKE ? OR 
				drainage_noc_jal_cc.house_building_society_name LIKE ? OR 
				drainage_noc_jal_cc.house_or_block_number LIKE ? OR 
				drainage_noc_jal_cc.wing LIKE ? OR 
				drainage_noc_jal_cc.floor LIKE ? OR 
				drainage_noc_jal_cc.road_name LIKE ? OR 
				drainage_noc_jal_cc.place_or_division_or_village LIKE ? OR 
				drainage_noc_jal_cc.taluka LIKE ? OR 
				drainage_noc_jal_cc.zone_value LIKE ? OR 
				drainage_noc_jal_cc.election_ward_no_value LIKE ? OR 
				drainage_noc_jal_cc.mobile_no LIKE ? OR 
				drainage_noc_jal_cc.email LIKE ? OR 
				drainage_noc_jal_cc.architect_name LIKE ? OR 
				drainage_noc_jal_cc.property_type_value LIKE ? OR 
				drainage_noc_jal_cc.computarized_property_no LIKE ? OR 
				drainage_noc_jal_cc.property_survey_no_or_city_survey_no LIKE ? OR 
				drainage_noc_jal_cc.mouje_gaon_name LIKE ? OR 
				drainage_noc_jal_cc.road_name_for_property LIKE ? OR 
				drainage_noc_jal_cc.place_division_gaon_for_property LIKE ? OR 
				drainage_noc_jal_cc.property_taluka LIKE ? OR 
				drainage_noc_jal_cc.pincode_for_property LIKE ? OR 
				drainage_noc_jal_cc.zone_property_value LIKE ? OR 
				drainage_noc_jal_cc.property_type_as_per_application_value LIKE ? OR 
				drainage_noc_jal_cc.property_usage LIKE ? OR 
				drainage_noc_jal_cc.property_usage_type_value LIKE ? OR 
				drainage_noc_jal_cc.upload_architech_affidavit LIKE ? OR 
				drainage_noc_jal_cc.upload_project_consultant_report LIKE ? OR 
				drainage_noc_jal_cc.upload_property_location_map LIKE ? OR 
				drainage_noc_jal_cc.rec_id LIKE ? OR 
				drainage_noc_jal_cc.user_id LIKE ? OR 
				drainage_noc_jal_cc.date LIKE ? OR 
				drainage_noc_jal_cc.certificate LIKE ? OR 
				drainage_noc_jal_cc.timestamp LIKE ? OR 
				drainage_noc_jal_cc.paid LIKE ? OR 
				drainage_noc_jal_cc.demand LIKE ?
			)";
			$search_params = array(
			...
			$this->view->search_template = "drainage_noc_jal_cc/search.php";
		}
		if(!empty($request->orderby)){
			...
			$db->orderBy("drainage_noc_jal_cc.id", ORDER_TYPE);
		}
		if($fieldname){
...
$record['timestamp'] = human_datetime($record['timestamp']);
		...
		$page_title = $this->view->page_title = "Drainage Noc Jal Cc";
		$this->render_view("drainage_noc_jal_cc/list.php", $data); //render the full page
	}
	/**
     * View record detail 
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("drainage_noc_jal_cc.id", $rec_id);; //select record based on primary key
		}
		$record = $db->getOne($tablename, $fields );
		if($record){
			$page_title = $this->view->page_title = "View  Drainage Noc Jal Cc";
		...
		return $this->render_view("drainage_noc_jal_cc/view.php", $record);
	}
	/**
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("applicant_name","upload_floor_plan","upload_full_potential_plan","house_building_society_name","house_or_block_number","floor","place_or_division_or_village","taluka","zone_id","mobile_no","email","architect_name","property_type_id","property_survey_no_or_city_survey_no","mouje_gaon_name","place_division_gaon_for_property","property_taluka","zone_property_id","property_type_as_per_application_id","property_usage","property_usage_type_id","wing","road_name","election_ward_no_id","computarized_property_no","road_name_for_property","pincode_for_property","upload_architech_affidavit","upload_project_consultant_report","upload_property_location_map","user_id","date","status");
...
$db->where("id", $rec_id);
$bool = $db->update("drainage_noc_jal_cc", $table_data);
//execute sql statement and return a single field value
...
$db->where("id", $rec_id);
$bool = $db->update("drainage_noc_jal_cc", $table_data1);
//execute sql statement and return a single field value
...
$db->where("id", $rec_id);
$bool = $db->update("drainage_noc_jal_cc", $table_data2);
//execute sql statement and return a single field value
...
$db->where("id", $rec_id);
$bool = $db->update("drainage_noc_jal_cc", $table_data3);
//execute sql statement and return a single field value
...
$db->where("id", $rec_id);
$bool = $db->update("drainage_noc_jal_cc", $table_data4);
//execute sql statement and return a single field value
...
$db->where("id", $rec_id);
$bool = $db->update("drainage_noc_jal_cc", $table_data5);
...
$receipt_no = "KDMC/DRAIN/JAL_CC/" . $application_id . "/" . $yr;
// Update the row with receipt_no
$db->where("id", $application_id);
$db->update("drainage_noc_jal_cc", ["application_no" => $receipt_no]);
		# End of after add statement
					$this->set_flash_msg("Record added successfully", "success");
					return	$this->redirect("drainage_noc_jal_cc");
				}
				else{
					$this->set_page_error();
				}
		...
		$page_title = $this->view->page_title = "Add New Drainage Noc Jal Cc";
		$this->render_view("drainage_noc_jal_cc/add.php");
	}
	/**
...
$modeldata['inspection'] = "-1";
			if($this->validated()){
				$db->where("drainage_noc_jal_cc.id", $rec_id);;
		...
		$page_title = $this->view->page_title = "Edit  Drainage Noc Jal Cc";
		if(!$data){
			$this->set_page_error();
		}
		return $this->render_view("drainage_noc_jal_cc/edit.php", $data);
	}
	/**
	...
	function delete($rec_id = null){
		...
		$db->where("drainage_noc_jal_cc.id", $arr_rec_id, "in");
...
$record['date'] = human_date($record['date']);
		...
		return $this->render_view("drainage_noc_jal_cc/cert_view.php", $record);
	}
	...
	function revert($rec_id = null, $formdata = null){
		...
		return $this->render_view("drainage_noc_jal_cc/revert.php", $data);
	}
}
