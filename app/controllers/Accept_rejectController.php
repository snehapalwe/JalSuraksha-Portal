...
class Accept_rejectController extends SecureController{
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("rec_id","action","remark","db_name");
...
$x=$db->getOne($modeldata['db_name'],"*");
    if($x['status']+0==1){
        
					$this->set_flash_msg("Appplication Cannot be Rejected", "danger");
...
$db->where("id",$modeldata['rec_id']);
$db->update($modeldata['db_name'],["status"=>$newstatus]);
$dbn = $modeldata['db_name'];
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id","rec_id","action","remark","db_name");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'rec_id' => 'required',
