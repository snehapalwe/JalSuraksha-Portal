<?php 
/**
 * Drainage_noc_mal_oc Page Controller
 * @category  Controller
 */
class Drainage_noc_mal_ocController extends SecureController{
	function __construct(){
		parent::__construct();
		$this->tablename = "drainage_noc_mal_oc";
	}
	/**
     * List page records
...
if(USER_ROLE==5){
	...
	# End of before list statement
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				drainage_noc_mal_oc.id LIKE ? OR 
				drainage_noc_mal_oc.status LIKE ? OR 
				drainage_noc_mal_oc.application_no LIKE ? OR 
				drainage_noc_mal_oc.applicant_name LIKE ? OR 
				drainage_noc_mal_oc.house_building_society_name LIKE ? OR 
				drainage_noc_mal_oc.house_or_block_number LIKE ? OR 
				drainage_noc_mal_oc.wing LIKE ? OR 
				drainage_noc_mal_oc.floor LIKE ? OR 
				drainage_noc_mal_oc.road_name LIKE ? OR 
				drainage_noc_mal_oc.place_or_division_or_village LIKE ? OR 
				drainage_noc_mal_oc.taluka LIKE ? OR 
				drainage_noc_mal_oc.zone_id LIKE ? OR 
				drainage_noc_mal_oc.election_ward_no_value LIKE ? OR 
				drainage_noc_mal_oc.mobile_no LIKE ? OR 
				drainage_noc_mal_oc.email LIKE ? OR 
				drainage_noc_mal_oc.property_type_value LIKE ? OR 
				drainage_noc_mal_oc.computarized_property_no LIKE ? OR 
				drainage_noc_mal_oc.property_survey_no_or_city_survey_no LIKE ? OR 
				drainage_noc_mal_oc.mouje_gaon_name LIKE ? OR 
				drainage_noc_mal_oc.road_name_for_property LIKE ? OR 
				drainage_noc_mal_oc.place_division_gaon_for_property LIKE ? OR 
				drainage_noc_mal_oc.property_taluka LIKE ? OR 
				drainage_noc_mal_oc.pincode_for_property LIKE ? OR 
				drainage_noc_mal_oc.zone_property_value LIKE ? OR 
				drainage_noc_mal_oc.property_type_as_per_application_value LIKE ? OR 
				drainage_noc_mal_oc.is_construction_commence_ceriticate_available LIKE ? OR 
				drainage_noc_mal_oc.commence_ceriticate_number LIKE ? OR 
				drainage_noc_mal_oc.commence_ceriticate_date LIKE ? OR 
				drainage_noc_mal_oc.property_usage LIKE ? OR 
				drainage_noc_mal_oc.property_usage_type_value LIKE ? OR 
				drainage_noc_mal_oc.architect_name LIKE ? OR 
				drainage_noc_mal_oc.upload_architech_affidavit LIKE ? OR 
				drainage_noc_mal_oc.upload_construction_permission_and_cerified_map_copy LIKE ? OR 
				drainage_noc_mal_oc.upload_architect_or_project_consultant_report LIKE ? OR 
				drainage_noc_mal_oc.upload_property_location_map LIKE ? OR 
				drainage_noc_mal_oc.upload_noc_copy_given_for_construction_permission LIKE ? OR 
				drainage_noc_mal_oc.upload_map_prepared_by_licensed_plumber LIKE ? OR 
				drainage_noc_mal_oc.rec_id LIKE ? OR 
				drainage_noc_mal_oc.user_id LIKE ? OR 
				drainage_noc_mal_oc.date LIKE ? OR 
				drainage_noc_mal_oc.certificate LIKE ? OR 
				drainage_noc_mal_oc.timestamp LIKE ? OR 
				drainage_noc_mal_oc.paid LIKE ? OR 
				drainage_noc_mal_oc.demand LIKE ? OR 
				drainage_noc_mal_oc.inspection LIKE ?
			)";
			$search_params = array(
			...
			$this->view->search_template = "drainage_noc_mal_oc/search.php";
		}
		if(!empty($request->orderby)){
			...
			$db->orderBy("drainage_noc_mal_oc.id", ORDER_TYPE);
		}
		if($fieldname){
		...
		$page_title = $this->view->page_title = "Drainage Noc Mal Oc";
		$this->render_view("drainage_noc_mal_oc/list.php", $data); //render the full page
	}
	/**
     * View record detail 
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("drainage_noc_mal_oc.id", $rec_id);; //select record based on primary key
		}
		$record = $db->getOne($tablename, $fields );
		if($record){
			$page_title = $this->view->page_title = "View  Drainage Noc Mal Oc";
		...
		return $this->render_view("drainage_noc_mal_oc/view.php", $record);
	}
	/**
...
$db->where("id", $rec_id);
$bool = $db->update("drainage_noc_mal_oc", $table_data);
//execute sql statement and return a single field value
...
$db->where("id", $rec_id);
$bool = $db->update("drainage_noc_mal_oc", $table_data1);
//execute sql statement and return a single field value
...
$db->where("id", $rec_id);
$bool = $db->update("drainage_noc_mal_oc", $table_data2);
//execute sql statement and return a single field value
...
$db->where("id", $rec_id);
$bool = $db->update("drainage_noc_mal_oc", $table_data3);
//execute sql statement and return a single field value
...
$db->where("id", $rec_id);
$bool = $db->update("drainage_noc_mal_oc", $table_data4);
//execute sql statement and return a single field value
...
$db->where("id", $rec_id);
$bool = $db->update("drainage_noc_mal_oc", $table_data5);
...
$receipt_no = "KDMC/DRAIN/MAL_OC/" . $application_id . "/" . $yr;
// Update the row with receipt_no
$db->where("id", $application_id);
$db->update("drainage_noc_mal_oc", ["application_no" => $receipt_no]);
		# End of after add statement
					$this->set_flash_msg("Record added successfully", "success");
					return	$this->redirect("drainage_noc_mal_oc");
				}
				else{
					$this->set_page_error();
				}
		...
		$page_title = $this->view->page_title = "Add New Drainage Noc Mal Oc";
		$this->render_view("drainage_noc_mal_oc/add.php");
	}
	/**
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id","applicant_name","house_building_society_name","house_or_block_number","floor","place_or_division_or_village","taluka","zone_id","mobile_no","email","architect_name","property_type_id","property_survey_no_or_city_survey_no","mouje_gaon_name","place_division_gaon_for_property","property_taluka","zone_property_id","property_type_as_per_application_id","property_usage","property_usage_type_id","wing","road_name","election_ward_no_id","computarized_property_no","road_name_for_property","pincode_for_property","is_construction_commence_ceriticate_available","commence_ceriticate_number","commence_ceriticate_date","upload_architech_affidavit","upload_construction_permission_and_cerified_map_copy","upload_architect_or_project_consultant_report","upload_property_location_map","upload_noc_copy_given_for_construction_permission","upload_map_prepared_by_licensed_plumber","user_id","date","status");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'applicant_name' => 'required',
...
$modeldata['status'] = "1";
			if($this->validated()){
				$db->where("drainage_noc_mal_oc.id", $rec_id);;
		...
		$page_title = $this->view->page_title = "Edit  Drainage Noc Mal Oc";
		if(!$data){
			$this->set_page_error();
		}
		return $this->render_view("drainage_noc_mal_oc/edit.php", $data);
	}
	/**
	...
	function delete($rec_id = null){
		...
		$db->where("drainage_noc_mal_oc.id", $arr_rec_id, "in");
	...
	function cert_view($rec_id = null, $value = null){
		...
		return $this->render_view("drainage_noc_mal_oc/cert_view.php", $record);
	}
}
